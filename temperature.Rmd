...
```{r}
porto_tolle <- get_yearly_data("porto_tolle")
auronzo <- get_yearly_data("auronzo")

inner_join(porto_tolle, auronzo, by = "year", suffix = c(".pt", ".au")) |>
    ggplot(aes(x = avg.pt, y = avg.au)) +
        ...
        labs(
            x = "Porto Tolle temperature (°C)",
            y = "Auronzo temperature (°C)",
            colour = "Year",
            title = paste(
                "Comparison of the average yearly temperatures in",
                "Porto Tolle and Auronzo"
            )
        )
```