...
We use JAGS to perform the three regressions, using uniform priors to fully
unbias the model comparison.
...
gridExtra::grid.arrange(
    plot_posteriors(as.mcmc(lin_chain), "a"),
...
gridExtra::grid.arrange(plot_posteriors(as.mcmc(quad_chain_2), "a"),
                        plot_posteriors(as.mcmc(quad_chain_2), "b"),
                        plot_posteriors(as.mcmc(quad_chain_2), "c"),
                        plot_posteriors(as.mcmc(quad_chain_2), "sigma"),
                        ncol = 2)
...
ggplot(lin_df) +
    ...
    labs(
        x = "Slope of the linear regression",
...
Assuming the increasing trend linear model is the best to describe the
yearly dataset, we average temperatures over a given value of years
interval.

Then we perform a linear Bayesian regression with JAGS as before to
estimate the temperature change rate and compare it with the one found by
SNPA for maxima, minima and averages.
...
In the case of maxima and averages 0 lies in the acceptance zone so we can say that the two measurements are compatible with a 95% confidence.
...
forecast_prediction <- function(data_type, station, pred_years, interv=5){
  ...
  if(data_type == "diff"){
    ...
    aa <- Arima(time_series, order = c(3,1,3), seasonal=c(1,1,2), lambda = 0)
    fc <- forecast(aa, level = c(95), h = pred_years*12)
  }
  
  gridExtra::grid.arrange(
                           autoplot(time_series) + theme_minimal(base_size = 8,
                                                                base_family = global_font) +
                             labs(x = "time",
                                  y = paste(data_type, "temperature trend"),
                                  title = "time series"),
                           ggAcf(time_series) + theme_minimal(base_size = 8,
                                                              base_family = global_font) +
                             labs(x = "lag",
                                  y = "acf",
                                  title = "acf of the time series"),
                           ggAcf(aa$residuals) +theme_minimal(base_size = 8,
                                                              base_family = global_font) +
                             labs(x = "lag",
                                  y = "acf",
                                  title = "acf of the residuals"),
                           autoplot(fc) + theme_minimal(base_size = 8,
                                                        base_family = global_font) +
                             labs(x = "time",
                                  y = paste(data_type, "temperature trend"),
...
```{r}
forecast_prediction("diff", "malo", 10)
forecast_prediction("yearly", "malo", 10)
forecast_prediction("period", "malo", 10)
